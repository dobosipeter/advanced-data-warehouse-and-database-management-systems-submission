...
\end{itemize}
\vspace{1em}
\textbf{Goal:} Build a production-ready DBMS-backed monitoring system with triggers,
transactions, and an operational GUI.
...
\begin{figure}
\centering
\includegraphics[width=0.85\textwidth]{../../diagrams/system_architecture.png}
\end{figure}
...
\begin{itemize}
  \item Hetzner Cloud VM (Ubuntu 24.04, 2 vCPU, 4 GB RAM)
  \item Docker Compose: DB + API + Frontend + Caddy
  \item Caddy handles automatic Let's Encrypt TLS
  \item GitHub Actions: push to \texttt{main} → rsync → docker compose up
  \item UFW firewall: only 22, 80, 443 open
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{../../diagrams/deployment_architecture.png}
\end{figure}
